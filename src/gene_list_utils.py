...
'''
Same as above but when each dictionary entry is a list of two elements.
'''
def print_dict_with_list(a_dictionary,dest=sys.stdout):
    for key in a_dictionary.keys():
        ...
        for line in hgnc_file.readlines():
            ...
            if mode == 'update':
                all_possible_names_and_symbols = map(str.strip,cols[colnames.index('Previous symbols')].split(',') + 
                    ...
                    cols[colnames.index('Alias symbols')].strip('"').split('|'))
